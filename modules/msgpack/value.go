...
func (j jsonValue) MarshalJSON() (data []byte, err error) {
	...
	case lua.LBool:
		data, err = msgpack.Marshal(bool(converted))
	case lua.LNumber:
		data, err = msgpack.Marshal(float64(converted))
	case *lua.LNilType:
		data = []byte(`null`)
	case lua.LString:
		data, err = msgpack.Marshal(string(converted))
	case *lua.LTable:
...
func ValueEncode(value lua.LValue) ([]byte, error) {
	return msgpack.Marshal(jsonValue{
		LValue:  value,
